...
pub fn interpn<T: Float>(
    ...
    match ndims {
        x if x == 1 => {
            MulticubicRectilinearRecursive::<'_, T, 1>::new(grids, vals, linearize_extrapolation)?
                .interp(obs, out)
        }
        x if x == 2 => {
            MulticubicRectilinearRecursive::<'_, T, 2>::new(grids, vals, linearize_extrapolation)?
                .interp(obs, out)
        }
        x if x == 3 => {
            MulticubicRectilinearRecursive::<'_, T, 3>::new(grids, vals, linearize_extrapolation)?
                .interp(obs, out)
        }
        x if x == 4 => {
            MulticubicRectilinearRecursive::<'_, T, 4>::new(grids, vals, linearize_extrapolation)?
                .interp(obs, out)
        }
        x if x == 5 => {
